...
| `ios/App/SuzakuWidget/PrivacyInfo.xcprivacy` | **SuzakuWidget** | プライバシーマニフェスト（審査必須。UserDefaults使用理由） |
...
4. **`PrivacyInfo.xcprivacy` を SuzakuWidget ターゲットに追加**（Target Membership を
   SuzakuWidget に）。拡張も UserDefaults を使うため、これが無いと審査で
   プライバシーマニフェスト不備の警告/リジェクトになりうる。
